...
jQuery(document).ready(function () {
    (function ($) {
        ...
        function country_state_select_list(country_selector, state_selector) {

            var temp_countries = {};
            var $country_select = $(country_selector);
            var $state_select = $(state_selector);

            var state_value = $state_select.val();
            var country_value = $country_select.val();

            //remove default options from state 
            //don't remove the -- None --
            $state_select.find("option:not([value=_none])").remove();
            ...
            try {
                for (var _iterator = countries[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var country_data = _step.value;
                    var states = country_data.states,
                        country = country_data.country;


                    temp_countries[country] = states;

                    //already added in field settings
                    //                let $country_option = $(`<option value="${country}">${country}</option>`);
            ...
            $country_select.on("change", country_change);

            //call country_change once
            country_change();
            //set value of state_select back to it's old value

            $state_select.val(state_value);

            //show states based on country
            function country_change() {

                var country = $country_select.val();

                var states = temp_countries[country];

                $all_states = $state_select.find("option[data-country]");
                $all_states.remove();
                ...
                if (states) {
                    ...
                    try {
                        for (var _iterator2 = states[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                            ...
                            var $state_option = $("<option data-country=\"" + country + "\" value=\"" + state + "\">" + state + "</option>");
        ...
        country_state_select_list('#edit-field-country', '#edit-field-state');
    })(jQuery);
});